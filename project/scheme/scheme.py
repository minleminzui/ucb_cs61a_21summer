...
def eval_all(expressions, env):
    ...
    while cur.rest != nil:
        ...
        cur = cur.rest
    return scheme_eval(cur.first, env, True)  # replace this with lines of your own code
    # END PROBLEM 7
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        cur = self
        while cur != None:
            if cur.bindings.get(symbol, None) != None:
...
# initial identifying symbol (i f, lambda, quote, ...). Its second argument is
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        to_bind = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(target.first, to_bind)
        return target.first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    cur = expressions
    while cur.rest != nil:
        if is_false_primitive(scheme_eval(cur.first, env)):
            return False
        cur = cur.rest
...
def do_or_form(expressions, env):
    ...
    cur = expressions
    while cur.rest != nil:
        if is_true_primitive(scheme_eval(cur.first, env)):
...
def make_let_frame(bindings, env):
    ...
    clause = bindings
    while clause != nil:
        validate_form(clause.first, 2, 2)
        names = Pair(clause.first.first, names)
        values = Pair(scheme_eval(clause.first.rest.first, env),values)
        clause = clause.rest
