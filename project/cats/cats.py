...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(paragrah):
        list_paragrah = split(remove_punctuation(paragrah))
        for top in topic:
            for item in list_paragrah:
                if lower(item) == top:
                    return True
        return False 
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    len_type, len_reference = len(typed_words), len(reference_words)
    if len_type == 0 and len_reference == 0:
        return 100.0
    if len_type == 0:
        return 0.0
    matched_word = 0
    for i in range(min(len_type, len_reference)):
        if typed_words[i] == reference_words[i]:
            matched_word += 1
    return matched_word * 100.0 / len_type
    # END PROBLEM 3
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if limit == -1:#it's too difficult to think of 
        return 0
    if len(start) == 0:
        return len(goal)
    elif len(goal) == 0:
...
def time_per_word(times_per_player, words):
    ...
    for i in range(player_num):
        ...
        for j in range(word_num):
            temp_time.append(times_per_player[i][j + 1] - times_per_player[i][j])
...
def fastest_words(game):
    ...
    for index in range(len(get_words(game))):
        faster_player = 0
        faster_time = 1<<30
        for i in range(len(get_times(game))):
            if get_times(game)[i][index] < faster_time:
                faster_time = get_times(game)[i][index]
                faster_player = i
        ans[faster_player].append(get_words(game)[index])
    return ans
    # END PROBLEM 10
