...
def closer_city(lat, lon, city_a, city_b):
    ...
    # if distance(make_city("target", lat, lon), city_a) > distance(make_city("target", lat, lon), city_b):
...
def berry_finder(t):
    ...
    True
    """
    ans = True if label(t) == "berry" else False
    for branch in branches(t):
        if label(branch) == "berry":
            return True
        else:
            ans = berry_finder(branch) or ans
    ...
    1
      2
        3
          ...
          2
    """
    # length = len(branches(t))
    # for i in range(length):
    #     if is_leaf(branches(t)[i]):
    #         # for item in leaves:
    #         #     branches(t)[i].append(tree())
    #         # subtree = []
    #         # for item in leaves:
    #         #     subtree.append(tree(item))
    #         branches(t)[i] = tree(label(branches(t)[i]), [tree(x) for x in leaves])
    #     else:
    #         sprout_leaves(branches(t)[i], leaves)
    # return t
    if is_leaf(t):
