def insert_into_all(item, nested_list):
    ...
    for lst in nested_list:
        ...
        insert_item_lst.append(new_lst)

    return insert_item_lst
...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        all_but_first_subsquence = subseqs(s[1:])    
        return all_but_first_subsquence + insert_into_all(s[0], all_but_first_subsquence)
...
def same_shape(t1, t2):
    ...
    if not t1 and t2:
        ...
        for i in range(len(branches(t1))):
             ans = ans and same_shape(branches(t1)[i], branches(t2)[i])
        return ans  
...
def add_trees(t1, t2):
    ...
    if  is_leaf(t1):
        ...
        fewer_braches, more_branches = sorted([branches(t1), branches(t2)], key=len)
        for i in range(len(more_branches) - len(fewer_braches)):
            fewer_braches.append(tree(0))
        return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zip(fewer_braches, more_branches)])
